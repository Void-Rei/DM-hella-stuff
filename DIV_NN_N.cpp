...
//Назначение: Частное от большего натурального числа на меньшее натуральное число.
//Тип возвращаемого значения - Натуральное
//Функция ссылается на следующие модули: SUB_NDN_N, DIV_NN_Dk.
...
DM_N DIV_NN_N (DM_N X1, DM_N X2)
{
   ...
   if (!NZER_N_B(X2))    //Если делитель равен нулю, выдает ошибку
      ...
      if (COM_NN_D(X1, X2)==2) //Если Первое число больше второго:
      {
        ...
        do
         {
            xCopy=SUB_NDN_N(xCopy, X2);  //Вычитаем из большего числа меньшее
